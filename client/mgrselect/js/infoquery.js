...
{id : "PUBLIC_STATUS",header : "公客属性",width:70,headAlign:'left',align : 'center',type :'string',exportable:false,renderer:opRender},
{id : "PUBLIC_STATUS",header : "操作",width:220,headAlign:'left',align : 'center',type :'string',exportable:false,renderer:handleRender},
...
function opRender(value, record, columnObj, grid, colNo, rowNo){
	if(value==1 || value == 2){
		return "<span style='color:green'>"+mystate[1]+"</span>";
	}else{
		return "<span style='color:#777777'>"+mystate[0]+"</span>";
	}
}
function handleRender(value ,record,columnObj,grid,colNo,rowNo){
	var id = record.CUST_ID;
	var userStatus = record.PUBLIC_FLAG;
	var saleStatus = record.CUST_SALE_STATUS;
	var projId = record.CUST_ORG_ID
	var a1='<a sign="aa" style="color:red;" href="javaScript:toNotPublic(\''+id+'\',\''+projId+'\')">【置为非公客】</a>|<a style="padding-left:5px" href="javaScript:adjust(\''+id+'\');">调整销售人员</a>';
	var a2='<a sign="aa" style="padding-right:12px;" href="javaScript:toPublic(\''+id+'\')">【置为公客】</a>|<a style="padding-left:5px" href="javaScript:adjust(\''+id+'\');">调整销售人员</a>';
	if(saleStatus == 3){//已经成交客户不能再搞为公客
		return '';
	}
	if(value==1 || value ==2)
		return a1;
...
function toPublic(custId){
	ajax.remoteCall(path+"/com.pytech.timesgp.web.dao.CustomerDao:toPublic",[custId],function(reply){
		var result = reply.getResult();
		dialogUtil.alert(result.msg,true);
		if(result.success){
			reloadGrid('grid');
...
function toNotPublic(custId,projId){
	ajax.remoteCall(path+"/com.pytech.timesgp.web.dao.CustomerDao:toNotPublic",[custId,projId],function(reply){
		var result = reply.getResult();
		dialogUtil.alert(result.msg,true);
		if(result.success){
			reloadGrid('grid');
