...
function posRender(val, record, columnObj, grid, colNo, rowNo) {
	if(record.GROUP_NAME=='周总计' || record.GROUP_NAME=='周目标')
		return record.USER_CODE;
	
	if(val==1)
		return "<font color='red'>组长</font>";
	else
		return "组员";
}
function nameRender(val, record, columnObj, grid, colNo, rowNo) {
	if(record.GROUP_NAME=='周总计')
		return record.USER_TYPE;
	if(record.GROUP_NAME=='周目标')
		return record.RECV_ORG_NAME;
	return val;
}
...
function exportPersonList(){
	...
	location.href = path + "/servlet/person_target.action?fileName=PersonTarget"+dt+params;
}
