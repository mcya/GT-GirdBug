...
function statusFanyi(val, record, columnObj, grid, colNo, rowNo) {
	if (val==0) return '<span>未审核</span>';
	if (val==1) return '<span>已审核</span>';
	return '<span></span>'
}
//预览
...
function shenhe() {
	...
	for(var i = 0;i<records.length;i++){
		...
		if (records[i].CUST_SALE_STATUS==2) {
			flagss = 2;
	...
	ajax.remoteCall(path+
		"/com.pytech.timesgp.web.dao.AttendanceDao:auditCust",
		[paramsData],
		function(reply){
			reloadGrid('grid');
